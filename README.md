...
### *Внимание! *.bat файлы изменены. Теперь не обязаельно создавать папку CMSIS только по пути C:\\. Скрипты научились работать с относительными путями. Единственное - избегайте кириллицу в пути. Создавать ярлыки теперь тоже не имеет смысла, все работает из коробки. Просто запускаем *.bat файл нужного проекта, он запросит запуститься от администратора. Ниже будет старенькая инструкция.*
