#   Build Analyzer v1.0 STM32F407VGT6
#   Автор: Волков Олег
#   Дата создания скрипта: 24.03.2025
...
$CCMRAM = $memoryData | Where-Object { $_.Name -eq "CCMRAM" }
$FLASH = $memoryData | Where-Object { $_.Name -eq "FLASH" }
$RAM = $memoryData | Where-Object { $_.Name -eq "RAM" }

$CCMRAM_Start_address = [int]$CCMRAM.Origin
$CCMRAM_End_address = [int]$CCMRAM.Origin + [int]$CCMRAM.Length 
...
$sections = @('.isr_vector', '.text', '.rodata', '.ARM', '.init_array', '.fini_array', '.data', '.bss', '._user_heap_stack', '.preinit_array')
...
# Расчет FLASH и RAM и CCMRAM
[int]$ccmramUsed = 0;
[int]$ramUsed = $sectionSizes['.data'] + $sectionSizes['.bss'] + $sectionSizes['._user_heap_stack']
[int]$flashUsed = $sectionSizes['.isr_vector'] + $sectionSizes['.rodata'] + $sectionSizes['.init_array'] + $sectionSizes['.fini_array'] + $sectionSizes['.text'] + $sectionSizes['.preinit_array']
    

# Размеры памяти (замените на свои значения)
$ccmramSize = [int]$ccmram_size_ld_file * 1024    
$ramSize = [int]$ram_size_ld_file * 1024    
$flashSize = [int]$flash_size_ld_file * 1024  

$ccmramFree = $ccmramSize - $ccmramUsed
...
$ccmramPercent = [math]::Round(($ccmramUsed / $ccmramSize) * 100, 2)
...
$data = @(
    [PSCustomObject]@{
        "Region    "        = "CCMRAM:" 
        "Start address    " = "0x$("{0:X8}" -f $CCMRAM_Start_address)"
        "End address    "   = "0x$("{0:X8}" -f $CCMRAM_End_address)"
        "Size      "        = "$([math]::Round($ccmramSize / 1024)) KB"
        "Free      "        = "$([math]::Round($ccmramFree / 1024, 2)) KB"
        "Used      "        = "$([math]::Round($ccmramUsed / 1024, 2)) KB"
        "Usage (%)"         = "$ccmramPercent%"
    },
    [PSCustomObject]@{
        "Region    "        = "RAM:" 
